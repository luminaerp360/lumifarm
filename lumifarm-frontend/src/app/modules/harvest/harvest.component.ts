...
export class HarvestComponent implements OnInit {
  ...
  loadCycles(): void {
    this.loading = true;
    this.cropCyclesService.getAll(this.page, this.limit, this.search || undefined, this.statusFilter || undefined).subscribe({
      next: (res) => {
        this.cycles = res.data;
        this.total = res.total;
        this.totalPages = res.totalPages;
        this.harvestCycles = this.cycles.filter(c => c.actualYield > 0 || c.actualHarvestDate || c.status === 'harvesting' || c.status === 'completed');
        this.loading = false;
        this.calcStats();
      },
  ...
  calcStats(): void {
    ...
    this.totalProjectedYield = this.cycles.reduce((s, c) => s + (c.projectedYield || 0), 0);
    this.totalActualYield = this.cycles.reduce((s, c) => s + (c.actualYield || 0), 0);
    this.totalRevenue = this.cycles.reduce((s, c) => s + (c.totalRevenue || c.actualCropValue || 0), 0);
    this.totalProfit = this.cycles.reduce((s, c) => s + (c.profitOrLoss || 0), 0);
  }

  onSearch(): void { this.page = 1; this.loadCycles(); }
  onFilterChange(): void { this.page = 1; this.loadCycles(); }
  goToPage(p: number): void { this.page = p; this.loadCycles(); }
  ...
  closeDetail(): void { this.showDetail = false; this.selectedCycle = null; }
  ...
  getStatusClasses(status: string): string {
    const map: Record<string, string> = {
      planning: 'bg-gray-100 text-gray-700 dark:bg-gray-700/30 dark:text-gray-400',
      active: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400',
      harvested: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
