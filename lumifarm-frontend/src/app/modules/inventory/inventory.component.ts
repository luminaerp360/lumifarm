...
export class InventoryComponent implements OnInit {
  ...
  loadDropdowns(): void {
    this.farmsService
      .getAll(1, 100)
      .subscribe({ next: (res) => (this.farms = res.data || []) });
    this.cropCyclesService
      .getAll(1, 100)
      .subscribe({ next: (res) => (this.cropCycles = res.data || []) });
  }

  onFarmChange(): void {
    if (this.form.farmId) {
      this.unitsService
        .getByFarm(this.form.farmId)
        .subscribe({ next: (plots: any[]) => (this.plots = plots || []) });
    } else {
      this.plots = [];
      this.form.plotId = "";
    }
  ...
  calcStats(): void {
    ...
    this.totalCost = this.inputs.reduce(
      (sum, i) => sum + (i.totalAmount || 0),
      0,
    );
    this.totalPaid = this.inputs.reduce(
  ...
  saveInput(): void {
    ...
    if (!tenantId) {
      this.formError = "No organization linked";
      return;
    }
    ...
    const payload = {
      ...
      amountPaid: Number(this.form.amountPaid || 0),
    };
    ...
    obs.subscribe({
      ...
      error: (e) => {
        this.formLoading = false;
        this.formError = e?.error?.message || "Failed to save";
      },
    });
  }
  ...
  deleteInput(id: string): void {
    ...
    this.farmInputsService.delete(id).subscribe(() => {
      this.closeDetail();
      this.loadInputs();
    });
  }
  ...
  getCategoryIcon(cat: string): string {
    const map: Record<string, string> = {
      ...
      herbicide: "fa-spray-can",
      fungicide: "fa-spray-can",
      insecticide: "fa-bug",
      manure: "fa-poo",
      fuel: "fa-gas-pump",
      equipment_hire: "fa-tractor",
      labor: "fa-users",
      transport: "fa-truck",
      packaging: "fa-box",
      irrigation: "fa-tint",
      land_preparation: "fa-mountain",
      other: "fa-ellipsis-h",
    };
