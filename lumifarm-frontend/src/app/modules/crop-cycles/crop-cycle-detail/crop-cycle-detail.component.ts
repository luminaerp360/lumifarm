...
export class CropCycleDetailComponent implements OnInit {
  ...
  load(id: string): void {
    this.CropCyclesService.getById(id).subscribe({
      next: (l) => {
        this.lease = l;
        this.loading = false;
        this.FarmFinanceService.getByLease(id).subscribe((p) => (this.payments = p));
      },
  ...
  activate(): void {
    ...
    this.CropCyclesService.activate(this.lease._id).subscribe((l) => (this.lease = l));
  }

  terminate(): void {
    if (!this.lease || !this.terminationReason) return;
    this.CropCyclesService.terminate(this.lease._id, this.terminationReason).subscribe((l) => {
      this.lease = l;
  ...
  deleteLease(): void {
    ...
    this.CropCyclesService.delete(this.lease._id).subscribe(() => this.goBack());
  }
