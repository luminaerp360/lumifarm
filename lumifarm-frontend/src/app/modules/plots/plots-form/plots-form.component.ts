...
import {
  PlotsService,
  Unit,
} from '../../../shared/services/plots/plots.service';
...
export class PlotsFormComponent implements OnInit, OnChanges {
  @Input() isOpen = false;
  ...
  loadFarms(): void {
    ...
    this.FarmsService.getAll(1, 1000).subscribe({
  ...
  onSubmit(): void {
    ...
    const request = this.unit?._id
      ? this.PlotsService.update(this.unit._id, payload)
      : this.PlotsService.create(payload);
